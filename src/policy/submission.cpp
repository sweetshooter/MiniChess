...
Move Player1::get_move(State *state, int depth){
  ...
  Move move = actions[0];
    for(auto it : actions){
      ...
      if(state->player) {
        int moveScore = Player1::alphabeta(nextState, depth-1, INT_MIN, INT_MAX, 1-state->player); //nextstate need to get lower point
        ...
        int moveScore = Player1::alphabeta(nextState, depth-1, INT_MIN, INT_MAX, 1-state->player);
        if(moveScore > bestScore){ //bestScore have updated
          bestScore = moveScore;
          move = it;
        }
...
int Player1::alphabeta(State* state, int depth, int a, int b, int isMaximizingPlayer){
